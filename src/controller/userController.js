...
const createUser = async function (req, res) {
    try {
        ...
        if (!title) return res.status(400).send({ status: false, msg: " title missing" })
        if (!name) return res.status(400).send({ status: false, msg: " name missing" })
        if (!phone) return res.status(400).send({ status: false, msg: " Please enter your contact details" })
        if (!email) return res.status(400).send({ status: false, msg: " email missing" })
        if (!password) return res.status(400).send({ status: false, msg: " password missing" })
